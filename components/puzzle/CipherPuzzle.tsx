...
import { CipherPuzzleData } from '@/types/puzzle';

interface CipherPuzzleProps {
  puzzle: CipherPuzzleData;
}

export default function CipherPuzzle({ puzzle }: CipherPuzzleProps) {
  return (
    <div className="space-y-5">
      ...
      {/* Encoded message */}
      <div>
        <p className="text-gray-500 text-xs uppercase tracking-widest mb-3 text-center">הצופן</p>
        <div className="flex justify-center gap-2 flex-wrap" dir="ltr">
          {puzzle.encodedMessage.map((symbol, i) => (
            <div
              key={i}
              className="w-12 h-12 flex items-center justify-center bg-gray-800 rounded-xl text-white text-2xl border border-gray-600"
            >
              {symbol}
            </div>
          ))}
      ...
      <p className="text-gray-400 text-sm text-center">פענחו את הצופן והקישו את המספרים בתיבה למטה</p>
    </div>
  );
}
